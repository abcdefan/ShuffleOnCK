...
- 保留 ClickHouse 官方维护的英文内容；除此之外，本项目新增或维护的 `CLAUDE.md` 内容统一使用中文，方便项目维护者直接阅读。

## 本地开发注意事项

- 命令行参数中包含 shell 通配符时必须使用引号。例如：
  `build/src/unit_tests_dbms '--gtest_filter=DistributedShuffleJoin.*'`。
  否则 `zsh` 会在测试程序接收到参数之前自行展开 `*`，导致命令执行失败。

- 如果在沙箱中运行 `ninja` 时只出现
  `ccache: error: Read-only file system`，
  应将其视为构建环境问题，而不是 C++ 编译错误。可以使用
  `CCACHE_DISABLE=1` 重新构建，或者申请相应权限后使用相同命令重新构建，
  让 `ccache` 能够访问其缓存目录。

- 汇报验证结果时，必须明确区分编译、单元测试、集成测试和性能测试。
  没有实际执行的测试类型，不能描述为已经通过。

- Praktika integration 的 `--test` 参数使用 integration test 目录名，例如：
  `--test test_distributed_shuffle_join`。
  不要使用 pytest 风格的 `::test_name` selector，因为 Praktika integration
  入口不支持这种格式。

...
20. 初步实现了左深树多表分布式 `shuffle join` 的多阶段执行路径。
   ...
   - 新增 `executeDistributedShuffleJoinLeftDeepPipeline`，复用现有单 stage 的 `Prepare -> Exchange -> Local JOIN -> Cleanup` 框架。非最终 stage 会创建普通 `Memory` output 表，将本地 `JOIN` 结果 `INSERT` 到该表，下一 stage 再把它作为左侧 source 重新 shuffle；最终 stage 仍通过 cluster Local `JOIN` result pipeline 返回给 initiator。

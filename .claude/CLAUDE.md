...
8. 测试覆盖
   ...
   - 当前覆盖 selector 分桶、从 analyzer info 创建 selector、cluster layout MVP 边界判断、Exchange side sink 创建、Exchange source SQL 生成、Exchange source `WHERE` 下推、显式 source database/table 的 Exchange source SQL 生成、post-join filter 的 Local `JOIN` SQL 生成、deterministic projection expression 的 Local `JOIN` SQL 生成、隐藏 `ORDER BY` 辅助列的 Local `JOIN` SQL 和最终 projection metadata、全局 `LIMIT` / `OFFSET` / `LIMIT WITH TIES` 与 `ORDER BY` execution plan 传递、`INNER ANY JOIN`、`SEMI JOIN`、`ANTI JOIN` strictness 以及 `LEFT JOIN` / `RIGHT JOIN` / `FULL JOIN` kind 到 Local `JOIN` SQL 的传递、缺失 source storage 保护、远端 Exchange payload 序列化/反序列化、远端 Exchange payload 中显式 source table 传递、执行计划生成、query id 缺失时的唯一 fallback 表名前缀、完整阶段顺序执行、`Prepare -> Exchange` 生命周期拆分、coordinator resource 析构 cleanup、owned query executor/coordinator resource 输入保护、coordinator resource attach 到 `QueryPipeline` 后 cleanup、Local `JOIN` pipeline 失败 cleanup、cluster Local `JOIN` pipeline 失败 cleanup、完整 pipeline 入口输入保护、source shard left/right 执行顺序、source shard Exchange 失败 cancel、当前 shard Exchange executor、system-query Exchange executor 保护、当前 shard Exchange executor 工厂校验、远端 Exchange 内部 `SYSTEM` 命令生成和解析、普通用户直接执行内部 `SYSTEM` 命令会被拒绝、`_shuffle_*` 表名和 SQL 生成、带 projection 的 Local `JOIN` SQL 生成、required columns 到 header 的转换、coordinator prepare/exchange/local join/cleanup、prepare 失败清理、cleanup `DROP` 失败后继续回收其他 shard、Exchange 失败清理、Local `JOIN` 失败清理。
   - 最近一次运行 `build/src/unit_tests_dbms '--gtest_filter=DistributedShuffleJoin.*'` 通过，56 个测试全部成功。
...
20. 初步实现了左深树多表分布式 `shuffle join` 的多阶段执行路径。

   - 新增 `DistributedShuffleJoinLeftDeepInfo` / `DistributedShuffleJoinLeftDeepStageInfo`，并通过 `tryAnalyzeDistributedShuffleJoinLeftDeep` 识别保守的左深树多表查询：所有输入都是直接 `StorageDistributed`，属于同一个 cluster，按 SQL 原始顺序执行，只接受直接列等值的 `INNER ALL JOIN` / `INNER ANY JOIN`。
   - 左深 analyzer 会把 `(((a JOIN b) JOIN c) JOIN d)` 拆成多个二元 stage，按后续 stage join key、最终 projection、全局 `ORDER BY` 和 `WHERE` 计算每个 stage output 需要保留的列；当不同输入表产生同名中间列时，会给 stage output 生成内部唯一列名并维护 source column 到内部列名的映射。当前仍保守拒绝 outer / semi / anti 多表语义、join reorder 和 cost model。
   - 左深多表路径现在支持 deterministic `WHERE`：每个 conjunct 会被放到所有引用表都已经可用的最早 stage。只引用单个原始输入表的条件会提前下推到该表所在 stage 的 source Exchange，并且这类只用于早期过滤的列不会再强制保留进后续 stage output schema；引用多个来源的条件会在对应 stage 的 target shard 本地 `JOIN` 后执行，而不是无条件等到 final stage。
   - 新增 `executeDistributedShuffleJoinLeftDeepPipeline`，复用现有单 stage 的 `Prepare -> Exchange -> Local JOIN -> Cleanup` 框架。非最终 stage 会创建普通 `Memory` output 表，将本地 `JOIN` 结果 `INSERT` 到该表，下一 stage 再把它作为左侧 source 重新 shuffle；最终 stage 仍通过 cluster Local `JOIN` result pipeline 返回给 initiator。非最终 output 表的 `CREATE TABLE` 和 cleanup fan-out 现在使用和 coordinator prepare/exchange 相同的并行 shard fan-out helper，避免 N-stage 查询在每个 stage 串行等待所有 shard 建表或清理 output；cleanup 路径遇到 0 shard 会直接 no-op，避免 best-effort 清理阶段产生无意义日志。
   - 非最终 stage 的 output 表会在发送 `CREATE TABLE` 前先加入待清理列表；如果 output 表在部分 shard 创建成功后其他 shard 抛 exception，外层 catch 仍会对所有 shard 执行 `DROP TABLE IF EXISTS`，避免 partial-create 泄漏。
   - `SystemQueryDistributedShuffleJoinExchangeExecutor` 和远端 Exchange payload 现在支持显式 source database/table，使后续 stage 可以读取上一 stage 的 output `Memory` 表，而不要求左侧一定是原始 `StorageDistributed`。
   - 过期 `_shuffle_*` 表的 opportunistic cleanup 现在会保护当前 Exchange 正在读取的显式 source table，除了当前 stage 的 left/right input 表，也跳过 payload 中的 `left_source.table` / `right_source.table`。这避免长查询里上一 stage output 已经过 TTL 后，被下一 stage 的远端 Exchange 在读取前误删。
   - `InterpreterSelectQueryAnalyzer::execute` 会先尝试左深多阶段路径，再回退已有二元路径；`executeQuery` 中为了避免普通 planner 提前触发分布式 product exception，也同步识别左深 analyzer。
   - 新增真实 integration case `test_left_deep_three_table_join_uses_multi_stage_shuffle`、`test_left_deep_using_chain_join_uses_multi_stage_shuffle` 和 `test_left_deep_four_table_join_uses_n_stage_shuffle`，覆盖不同 join key 的三表和四表左深链式 `JOIN`，并确认 `_shuffle_*` 表清理完成；三表用例还覆盖了前序中间结果侧过滤、当前右表侧过滤和跨 final stage 两侧 post-join filter 混合出现的 `WHERE`，未显式 strictness 的普通 `INNER JOIN`，最终 projection 表达式同时引用上一 stage output 和当前右表，全局 `ORDER BY` / `LIMIT` / `OFFSET`、最终 projection 不输出排序 key 的隐藏 `ORDER BY`，以及带隐藏排序表达式的 `LIMIT WITH TIES`，同时输出 `a.id AS a_id` / `b.id AS b_id` 的重复中间列名场景，并进一步覆盖 `WHERE a.id + b.id = ...` 这类同时引用上一 stage output 中两个同名来源列的 source filter 映射，第一 stage 使用 `USING (id)`、第二 stage 使用不同 key 的左深多阶段查询，后续 stage 使用 `USING (bucket)` 从上一 stage output 与第三张表继续 join，以及连续两段 `USING (id)` 的同 key 左深链。三表用例现在还会通过 `system.query_log` 检查每个 source shard 对 stage 0 的原始 left/right 本地表、stage 1 的 output `Memory` 表和第三张原始本地表各只扫描一次，证明左深路径仍是按 stage push-based shuffle，而不是重复拉取；同一用例还新增 `distributed_shuffle_join_table_ttl_ms = 1` 的查询，覆盖上一 stage output 表已过期时，下一 stage Exchange 的 opportunistic cleanup 不能删除当前正在读取的 output source。新增 `test_left_deep_outer_join_preserves_regular_distributed_join_exception`、`test_left_deep_semi_join_preserves_regular_distributed_join_exception`、`test_left_deep_expression_key_preserves_regular_distributed_join_exception` 和 `test_left_deep_compound_key_preserves_regular_distributed_join_exception`，确认当前暂未支持的多表 `LEFT ALL JOIN` / `LEFT SEMI JOIN`、表达式 join key 和复合 `ON` 条件不会误进左深 shuffle hook，而是保留普通 planner 的 distributed product exception，且不创建 `_shuffle_*` 表。三表用例末尾还新增了第二阶段右侧 source 缺失的失败路径覆盖：第一阶段 output 已创建后，第二阶段 Exchange 失败时仍应清理所有 `_shuffle_*` input/output 表。
   - 三表 integration 新增了 source filter 提前下推覆盖：`WHERE a.id >= ... AND b.b_value = ... AND c.bucket = ...` 会通过 `system.query_log` 确认分别出现在 `a`、`b`、`c` 三个原始 local source query 的 `WHERE` 中，避免单表过滤等到后续 stage output 后才执行；另有只输出 `a.id` 但用 `b.b_value` 过滤的查询，覆盖早期过滤列不再保留进 stage output schema 的列裁剪路径；还有只输出 `c.c_value` 但用 `a.id + b.id = ...` 过滤的查询，覆盖跨 stage 0 两侧输入的 post-join filter 会在 stage 0 执行，并且不会要求把对应早期列带到 stage 1 output。
   - 新增 `test_left_deep_aggregation_preserves_regular_distributed_join_exception`、`test_left_deep_distinct_preserves_regular_distributed_join_exception` 和 `test_left_deep_parallel_replicas_preserves_regular_distributed_join_exception`，确认当前暂未支持的左深多表聚合 / `DISTINCT` 查询以及 parallel replicas 设置不会误进 shuffle hook，而是保留普通 planner 的 distributed product exception，且不创建 `_shuffle_*` 表。
   - 三表 integration 还会在第一阶段右表插入重复 key 后执行左深 `ANY INNER JOIN`，只投影 `id` 并验证结果仍为 `1, 2, 3, 4`，确认多阶段路径会把第一阶段 strictness 传到 target shard Local `JOIN`，不会退化成 `ALL` 后多返回重复行。现在也覆盖第二阶段 `ANY INNER JOIN`：第三张表插入重复 key 后，`((a INNER ALL JOIN b) ANY INNER JOIN c)` 仍只返回 `1, 2, 3, 4`，确认后续 stage 的 strictness 同样会传到对应 Local `JOIN`。
   - 最近补充了左深最终全局排序和 projection 的边界覆盖：当 projection 中存在重复可见列名时，`collectOrderBy` 会为对应 `ORDER BY` 表达式生成隐藏 `_shuffle_order_by_N` 列，避免 `SortDescription` 按重复结果名排序产生歧义；最终隐藏列移除逻辑现在按 header 顺序匹配可见列，保留重复可见列名而不会被 name map 折叠。`collectColumnsRequiredAfterStage` 现在显式收集 `SortNode` 的排序表达式列，确保只用于最终全局排序的早期列会进入对应 stage output schema；如果 `ORDER BY` 子节点不是当前支持的 `SortNode`，左深 analyzer 会直接放弃并回退普通 planner，而不是静默跳过依赖列。新增 `ExecutionPlanAllowsHiddenProjectionWithDuplicateVisibleNames` 和 `CreatesLeftDeepStagePlansWithDuplicateVisibleNamesAndHiddenOrderBy` gtest，以及 `test_left_deep_duplicate_projection_hidden_order_uses_shuffle_join` integration，用重复 alias、隐藏排序表达式、`LIMIT WITH TIES` 和 query log source scan 计数验证该左深两阶段路径；integration 中的 `LIMIT 3 WITH TIES` 用例会让隐藏排序表达式的最后一个 tie group 一起返回，同时最终输出仍只保留两个重复可见列。
   - 最近还补充了左深多阶段 lifecycle 覆盖：三表 integration 增加第一阶段 Local `JOIN` 写入 output 表时抛 exception、最终阶段 Local `JOIN` 抛 exception 后清理 `_shuffle_*` 表的断言；四表 N-stage integration 现在通过 `system.query_log` 断言三阶段 source scan 形态，确认 stage 0 读取两个原始 local 表，stage 1 / stage 2 的左侧各读取一次上一阶段 `_output` 表，后续右侧分别读取第三、第四张原始 local 表。
   - 本轮新增 `ExchangeSourceQueryUsesExplicitSource` 和 `RemoteExchangePayloadUsesExplicitSources` gtest，验证后续 stage 可以把上一 stage output `Memory` 表作为显式 source 下发到远端 Exchange。后续又新增 `createDistributedShuffleJoinLeftDeepStagePlans` helper、`CreatesLeftDeepStagePlansWithIntermediateOutputSource`、`CreatesLeftDeepStagePlansForNStages`、`CreatesLeftDeepStagePlansWithExpiringOutputTables`、`CreatesLeftDeepStagePlansPreserveFinalGlobalPostProcessing` 和 `CreatesLeftDeepStagePlansPruneEarlyFilterOnlyColumns` gtest，把左深 stage 的表名、非最终 output header、下一 stage left source、N-stage output 逐级传递、output 表名携带 expiration marker 并可被过期清理解析、final stage 的隐藏 projection、全局 `ORDER BY` 和 `LIMIT WITH TIES` 信息传递，以及早期 post-join filter 使用的列不会泄漏到下一 stage output header 覆盖在不依赖 Docker 的单测里；真实 `executeDistributedShuffleJoinLeftDeepPipeline` 现在也复用同一个 stage plan helper，避免测试和执行路径各自计算 stage source/table。`buildStageOutputProjection` 现在对当前 stage right side 输出到下一 stage 的列也会查询 `right_column_name_mappings`，与 left side 的内部列名映射行为保持一致。`createDistributedShuffleJoinLeftDeepStagePlans` 现在为每个 stage 生成带 `_stage_<index>` 的独立 `DistributedShuffleJoinExchangeId`，不再用 `join_id * 1000 + stage_index` 这类隐式容量编码；新增 `CreatesLeftDeepStagePlansUseStageSpecificIdsPastOldFixedCapacity` gtest，用 1001 个 stage 验证 input/output 表名仍唯一，避免极端 N-stage 或未来多 hook 场景下的表名碰撞假设。stage-specific id 拼接现在显式拒绝空 `initial_query_id`，避免 `_stage_<index>` 后缀绕过底层 `_shuffle_*` 表名 helper 对 query id 的非空约束，并用 `CreatesLeftDeepStagePlansRejectsEmptyInitialQueryId` gtest 覆盖。左深 stage plan helper 现在还会拒绝 top-level `shard_count = 0` 以及任一 stage 的 `shard_count` 与 top-level 不一致，避免 coordinator 和 Exchange payload/selector 使用不同 shard 数；新增 `CreatesLeftDeepStagePlansRejectsMismatchedShardCount` gtest 覆盖。当 top-level `cluster_name` 非空时，helper 也会要求每个 stage 的 `cluster_name` 一致，避免远端 `SYSTEM` Exchange payload 按错误 cluster 执行；新增 `CreatesLeftDeepStagePlansRejectsMismatchedClusterName` gtest 覆盖。helper 现在还会把每个 stage plan 的 `shuffle_database` 归一化为 top-level `shuffle_database`，确保 stage metadata 与实际 `_shuffle_*` 表创建位置一致；新增 `CreatesLeftDeepStagePlansNormalizeStageShuffleDatabase` gtest 覆盖。真实 `executeDistributedShuffleJoinLeftDeepPipeline` 入口现在会检查 analyzer 提供的 `shard_count` 与第一张表 cluster 的 shard 数一致，避免后续 coordinator 按错误 shard 数下发 stage。
   - 最新补充：真实 `executeDistributedShuffleJoinLeftDeepPipeline` 入口也会检查 analyzer 提供的 `cluster_name` 与第一张表 cluster 一致，避免后续 coordinator 按错误 cluster 元数据下发 stage。
   - 本轮补上 `createDistributedShuffleJoinLeftDeepStagePlans` 的 stage metadata / output schema guard：top-level `cluster_name` 必须非空，所有 stage 的 `cluster_name` 必须与 top-level 一致；非最终 stage 必须携带 `output_columns`，必须和下一 stage 的左侧输入 schema 逐列一致，不能带隐藏 projection 列，并且非最终 stage Local `JOIN` 的可见 projection 列名必须和 output schema 列名一致；final stage 必须没有 `output_columns`，因为它直接返回结果 pipeline 而不会创建 output 表。违反这些 invariant 会在计划构造阶段抛 exception，而不是等到后续 `CREATE TABLE` / `INSERT`、远端 Exchange payload 或下一 stage Exchange 才暴露。同步新增空 `cluster_name`、空 `shuffle_database`、0 shard、单 stage、final stage 带 output schema、空非最终 output schema、output schema 与下一 stage left input schema 不一致、projection 与 output schema 不一致，以及非最终 stage 带隐藏 projection 的 gtest 覆盖。
   - `createDistributedShuffleJoinExecutionPlan` 现在会拒绝非法全局后处理元数据：`OFFSET` 不能没有 `LIMIT`，`LIMIT WITH TIES` 不能没有 `LIMIT`，并且 `LIMIT WITH TIES` 必须带 `ORDER BY`。这些形态正常会被 analyzer 提前过滤，但 helper 入口也显式守住 invariant，避免测试或后续调用方构造出无法正确执行的 plan。
   - `createDistributedShuffleJoinExecutionPlan` 和 Exchange source query helper 现在也会拒绝不完整的显式 source metadata：`database` / `table` 要么同时提供，要么同时为空并回退到 `StorageDistributed` 的 remote source。这个 guard 主要保护左深后续 stage，避免上一 stage output source 只丢失一半字段时静默回退到原始 distributed storage，导致错误到远端 Exchange 阶段才暴露。
   - 修复左深 `WHERE` conjunct 拆分的一个稳健性问题：`collectWhereConjuncts` 现在除了检查 `FunctionNode` 名称，也会通过转换回 `ASTFunction` 兜底识别逻辑 `and`，避免 analyzer/optimizer 后的函数名形态变化导致整个 `AND` 条件被当成单个 final-stage post-join filter，从而失去单表 source filter 下推。integration 里的左深 filter pushdown 用例现在继续通过 `system.query_log` 验证后续 source query 带 `WHERE`；重复可见列名 + hidden `ORDER BY` 用例改为使用合法的自然重复列名，避免用户查询本身先被 analyzer 的重复 alias 检查拒绝。
   - `collectUsingJoinKeys` 现在不再直接使用 analyzer 为 `USING` 生成的合并列 source，而是按当前 stage 的 left/right source 集合从对应 `TableNode` storage snapshot 中解析同名 key 列，生成带真实 source 的 key column；这修复了后续 stage 使用 `USING (bucket)` 时 analyzer 无法把 key 追溯到上一 stage output 中 `b.bucket` 的问题。对于连续 `USING (id)` 的同 key 左深链，如果当前 stage 左侧多个 source 都有同名 key，会选取第一个可用 source 作为 shuffle key，避免误判为无法分析。
   - 本轮 `ninja -C build programs/clickhouse src/unit_tests_dbms` 编译通过，日志见 `build/build_left_deep_shuffle_join.log`；新增 helper 覆盖后 `ninja -C build src/unit_tests_dbms` 也通过，日志见 `build/build_left_deep_shuffle_join_gtest_additions.log`。加入左深 `WHERE` 支持后，`ninja -C build programs/clickhouse src/unit_tests_dbms` 通过，日志见 `build/build_left_deep_shuffle_join_where.log`；加入 plain / `ANY` 左深查询覆盖后，`ninja -C build programs/clickhouse src/unit_tests_dbms` 也通过，日志见 `build/build_left_deep_shuffle_join_plain_any.log`；加入重复中间列名映射后，`ninja -C build programs/clickhouse src/unit_tests_dbms` 通过，日志见 `build/build_left_deep_shuffle_join_duplicate_columns.log`；加入左深 stage plan helper 后，`ninja -C build src/unit_tests_dbms` 和 `ninja -C build programs/clickhouse` 都通过，日志见 `build/build_left_deep_shuffle_join_stage_plan.log` 和 `build/build_left_deep_shuffle_join_stage_plan_clickhouse.log`。新增左深 `USING` 和 source scan-count integration 覆盖后，`/usr/bin/python3 -c "import ast, pathlib; ast.parse(...)"` 只读语法检查通过；`build/src/unit_tests_dbms '--gtest_filter=DistributedShuffleJoin.*'` 为 56 个用例全部通过，日志见 `build/test_left_deep_shuffle_join_scan_count_gtest.log`。修复后续 stage `USING` 后，`ninja -C build programs/clickhouse src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 也通过，日志见 `build/build_left_deep_shuffle_join_later_using.log` 和 `build/test_left_deep_shuffle_join_later_using_gtest.log`。新增第二 stage `ANY` integration 覆盖后，Python 只读语法检查和 `DistributedShuffleJoin.*` gtest 通过，日志见 `build/test_left_deep_shuffle_join_second_stage_any_gtest.log`。新增连续 `USING (id)` 左深链覆盖后，Python 只读语法检查、`ninja -C build programs/clickhouse src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 通过，日志见 `build/build_left_deep_shuffle_join_using_chain.log` 和 `build/test_left_deep_shuffle_join_using_chain_gtest.log`。新增 N-stage plan gtest 后，`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 通过，日志见 `build/build_left_deep_shuffle_join_n_stage_plan_gtest.log` 和 `build/test_left_deep_shuffle_join_n_stage_plan_gtest.log`。修复 output 表 partial-create cleanup 后，`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 通过，日志见 `build/build_left_deep_shuffle_join_output_create_cleanup.log` 和 `build/test_left_deep_shuffle_join_output_create_cleanup_gtest.log`。新增 expiring output table plan gtest 后，`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 通过，日志见 `build/build_left_deep_shuffle_join_expiring_output_gtest.log` 和 `build/test_left_deep_shuffle_join_expiring_output_gtest.log`。补上 opportunistic cleanup 保护当前 Exchange source 后，`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 再次通过，日志见 `build/build_left_deep_shuffle_join_cleanup_protect_sources.log` 和 `build/test_left_deep_shuffle_join_cleanup_protect_sources_gtest.log`。复跑 `CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `build/src/unit_tests_dbms '--gtest_filter=DistributedShuffleJoin.*'` 均通过，日志见 `build/build_left_deep_shuffle_join_final_gtest.log` 和 `build/test_left_deep_shuffle_join_final_gtest.log`。新增 final stage 全局后处理 gtest 后再次复跑，当前为 59 个用例全部成功，最新日志见 `build/build_left_deep_shuffle_join_final_post_processing_gtest.log` 和 `build/test_left_deep_shuffle_join_final_post_processing_gtest.log`。加入左深单表 `WHERE` 提前下推后，Python AST 检查和 `git diff --check` 通过，`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 以及 `build/src/unit_tests_dbms '--gtest_filter=DistributedShuffleJoin.*'` 也通过，最新日志见 `build/build_left_deep_shuffle_join_filter_pushdown.log` 和 `build/test_left_deep_shuffle_join_filter_pushdown_gtest.log`。加入多来源 `WHERE` 最早 stage 执行后，`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` 59 个 gtest 继续通过，日志见 `build/build_left_deep_shuffle_join_early_filter_stage.log` 和 `build/test_left_deep_shuffle_join_early_filter_stage_gtest.log`。新增早期 filter 列裁剪 gtest 后，`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` 60 个 gtest 通过，日志见 `build/build_left_deep_shuffle_join_filter_prune_gtest.log` 和 `build/test_left_deep_shuffle_join_filter_prune_gtest.log`。最近新增重复可见列名 hidden `ORDER BY`、stage source query 和 N-stage query log 覆盖后，`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `build/src/unit_tests_dbms '--gtest_filter=DistributedShuffleJoin.*'` 再次通过，当前为 61 个用例全部成功，最新日志见 `build/build_left_deep_shuffle_join_n_stage_querylog.log` 和 `build/test_left_deep_shuffle_join_n_stage_querylog_gtest.log`。新增左深最终 stage 重复可见列名 + 尾部隐藏 `ORDER BY` + `LIMIT WITH TIES` stage plan 覆盖后，`git diff --check`、integration Python AST 检查、`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `build/src/unit_tests_dbms '--gtest_filter=DistributedShuffleJoin.*'` 通过，当前为 62 个用例全部成功，日志见 `build/build_left_deep_shuffle_join_duplicate_hidden_ties.log` 和 `build/test_left_deep_shuffle_join_duplicate_hidden_ties_gtest.log`。显式收集 `SortNode` 排序表达式列并对非 `SortNode` 子节点直接回退后，`git diff --check`、integration Python AST 检查、`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 继续通过，当前仍为 62 个用例全部成功，日志见 `build/build_left_deep_shuffle_join_order_dependency_fallback.log` 和 `build/test_left_deep_shuffle_join_order_dependency_fallback_gtest.log`。补齐 right side output projection 映射后，`git diff --check`、integration Python AST 检查、`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 继续通过，当前仍为 62 个用例全部成功，日志见 `build/build_left_deep_shuffle_join_right_output_mapping.log` 和 `build/test_left_deep_shuffle_join_right_output_mapping_gtest.log`。非最终 output 表创建和 cleanup 改为并行 shard fan-out 后，`git diff --check`、integration Python AST 检查、`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 继续通过，当前仍为 62 个用例全部成功，日志见 `build/build_left_deep_shuffle_join_parallel_output_cleanup.log` 和 `build/test_left_deep_shuffle_join_parallel_output_cleanup_gtest.log`。新增重复可见 alias + hidden `ORDER BY` + `LIMIT WITH TIES` 的真实 integration 覆盖后，`git diff --check`、integration Python AST 检查、`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 继续通过，当前仍为 62 个用例全部成功，日志见 `build/build_left_deep_shuffle_join_duplicate_hidden_ties_integration.log` 和 `build/test_left_deep_shuffle_join_duplicate_hidden_ties_integration_gtest.log`。并行 fan-out helper 增加 0 shard no-op guard 后，`git diff --check`、integration Python AST 检查、`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 继续通过，当前仍为 62 个用例全部成功，日志见 `build/build_left_deep_shuffle_join_parallel_fanout_zero_guard.log` 和 `build/test_left_deep_shuffle_join_parallel_fanout_zero_guard_gtest.log`。左深 stage 表名改为 stage-specific `DistributedShuffleJoinExchangeId` 并新增 1001-stage 表名唯一性 gtest 后，`git diff --check`、integration Python AST 检查、`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 继续通过，当前为 63 个用例全部成功，日志见 `build/build_left_deep_shuffle_join_stage_specific_many_stage.log` 和 `build/test_left_deep_shuffle_join_stage_specific_many_stage_gtest.log`。补上空 `initial_query_id` guard 后，`git diff --check`、`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 继续通过，当前为 64 个用例全部成功，日志见 `build/build_left_deep_shuffle_join_empty_stage_query_id_guard.log` 和 `build/test_left_deep_shuffle_join_empty_stage_query_id_guard_gtest.log`。补上左深 stage plan `shard_count` 一致性 guard 后，`git diff --check`、`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 继续通过，当前为 65 个用例全部成功，日志见 `build/build_left_deep_shuffle_join_shard_count_guard.log` 和 `build/test_left_deep_shuffle_join_shard_count_guard_gtest.log`。补上左深 stage plan `cluster_name` 一致性 guard 后，`git diff --check`、`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 继续通过，当前为 66 个用例全部成功，日志见 `build/build_left_deep_shuffle_join_cluster_name_guard.log` 和 `build/test_left_deep_shuffle_join_cluster_name_guard_gtest.log`。归一化左深 stage plan `shuffle_database` 后，`git diff --check`、`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 继续通过，当前为 67 个用例全部成功，日志见 `build/build_left_deep_shuffle_join_normalize_stage_database.log` 和 `build/test_left_deep_shuffle_join_normalize_stage_database_gtest.log`。补上左深 pipeline 入口 cluster shard 数校验后，`git diff --check`、`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 继续通过，当前仍为 67 个用例全部成功，最新日志见 `build/build_left_deep_shuffle_join_pipeline_shard_guard.log` 和 `build/test_left_deep_shuffle_join_pipeline_shard_guard_gtest.log`。
   - 最新验证：补上左深 pipeline 入口 `cluster_name` 校验后，`git diff --check`、`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 继续通过，当前仍为 67 个用例全部成功，日志见 `build/build_left_deep_shuffle_join_pipeline_cluster_guard.log` 和 `build/test_left_deep_shuffle_join_pipeline_cluster_guard_gtest.log`。
   - 最新验证：补上 left-deep stage plan 必填 `cluster_name` guard 后，`git diff --check`、`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 继续通过，当前为 76 个用例全部成功，日志见 `build/build_left_deep_shuffle_join_cluster_required_guard.log` 和 `build/test_left_deep_shuffle_join_cluster_required_guard_gtest.log`。
   - 最新验证：补上 execution plan `LIMIT` / `OFFSET` 元数据 guard 后，`git diff --check`、`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 继续通过，当前为 77 个用例全部成功，日志见 `build/build_left_deep_shuffle_join_limit_metadata_guard.log` 和 `build/test_left_deep_shuffle_join_limit_metadata_guard_gtest.log`。
   - 最新验证：补上显式 source metadata 成对校验后，`git diff --check`、`CCACHE_DISABLE=1 ninja -C build src/unit_tests_dbms` 和 `DistributedShuffleJoin.*` gtest 继续通过，当前为 79 个用例全部成功，日志见 `build/build_left_deep_shuffle_join_source_metadata_guard.log` 和 `build/test_left_deep_shuffle_join_source_metadata_guard_gtest.log`。
   - 最新验证：修复左深 `WHERE` conjunct 拆分和 integration 重复列名用例后，`git diff --check`、Python 语法检查、`CCACHE_DISABLE=1 ninja -C build programs/clickhouse src/unit_tests_dbms`、`DistributedShuffleJoin.*` gtest 和完整 Praktika `test_distributed_shuffle_join` 均通过；gtest 当前为 79 个用例全部成功，完整 integration 当前为 33 个用例全部成功。日志见 `build/build_left_deep_shuffle_join_and_conjunct_fix_clickhouse.log`、`build/test_left_deep_shuffle_join_and_conjunct_fix_gtest.log` 和 `build/test_integration_distributed_shuffle_join_left_deep_and_fix_rebuilt.log`。
   - 当前会话中完整 Praktika integration 仍未能本地执行到 pytest：普通运行和提升运行都因 `ci/tmp/environment.json` 权限失败，尝试 `chown -R zhangzhifan:zhangzhifan ci/tmp` 也被系统拒绝。相关日志见 `build/test_left_deep_shuffle_join_current_integration.log`、`build/test_left_deep_shuffle_join_stage_source_query_integration.log` 和 `build/test_left_deep_shuffle_join_escalated_integration.log`；因此最近新增的 left-deep integration 覆盖已通过 Python AST 检查，但仍需在可写 `ci/tmp` / 可用 Docker 的环境里跑完整 `test_distributed_shuffle_join`。
   - 由于当前会话的 Docker/Praktika 运行需要额外权限，本轮使用本地两节点 server 手动验证真实路径：三表查询结果见 `build/test_left_deep_shuffle_join_manual_cluster.log`，四表 N-stage 查询结果见 `build/test_left_deep_shuffle_join_manual_cluster_4table.log`；新增左深 `WHERE` 查询返回预期的单行 `id = 4`，日志见 `build/test_left_deep_shuffle_join_where_manual_query.log`；普通 `INNER JOIN` 和 `ANY INNER JOIN` 左深查询分别见 `build/test_left_deep_shuffle_join_plain_manual_query.log` 与 `build/test_left_deep_shuffle_join_any_manual_query.log`；重复中间列名查询返回 `a_id` / `b_id` / `c_value` 预期结果，日志见 `build/test_left_deep_shuffle_join_duplicate_columns_manual_query.log`；左深 `USING (id)` 查询返回预期 4 行，日志见 `build/test_left_deep_shuffle_join_using_manual_query.log`；用于 scan-count 的左深查询结果正确，日志见 `build/test_left_deep_shuffle_join_scan_count_manual_query.log`，但该手工 server 配置没有 `system.query_log` 表，无法在本地手工环境验证 query_log 计数，检查日志见 `build/test_left_deep_shuffle_join_scan_count_manual_query_log_table_node1.log` 和 `build/test_left_deep_shuffle_join_scan_count_manual_query_log_table_node2.log`；后续 stage `USING (bucket)` 修复后也用本地两节点 server 跑通，日志见 `build/test_left_deep_shuffle_join_later_using_manual_fixed_query.log`；第二 stage `ANY` 也已手工跑通，只返回 `1, 2, 3, 4`，日志见 `build/test_left_deep_shuffle_join_second_stage_any_manual_query.log`；连续 `USING (id)` 左深链返回预期 4 行，日志见 `build/test_left_deep_shuffle_join_using_chain_manual_query.log`；多表 `LEFT ALL JOIN` fallback 返回预期 double-distributed exception，日志见 `build/test_left_deep_shuffle_join_outer_fallback_manual_query.log`；左深三表全局 `ORDER BY id DESC LIMIT 2 OFFSET 1` 返回预期的 `id = 3, 2`，日志见 `build/test_left_deep_shuffle_join_order_limit_manual_query.log`。两个节点的残留表检查日志 `build/test_left_deep_shuffle_join_manual_node1_shuffle_tables_after.log`、`build/test_left_deep_shuffle_join_manual_node2_shuffle_tables_after.log`、`build/test_left_deep_shuffle_join_where_manual_node1_shuffle_tables.log`、`build/test_left_deep_shuffle_join_where_manual_node2_shuffle_tables.log`、`build/test_left_deep_shuffle_join_plain_any_manual_node1_shuffle_tables.log`、`build/test_left_deep_shuffle_join_plain_any_manual_node2_shuffle_tables.log`、`build/test_left_deep_shuffle_join_duplicate_columns_manual_node1_shuffle_tables.log`、`build/test_left_deep_shuffle_join_duplicate_columns_manual_node2_shuffle_tables.log`、`build/test_left_deep_shuffle_join_using_manual_node1_shuffle_tables.log`、`build/test_left_deep_shuffle_join_using_manual_node2_shuffle_tables.log`、`build/test_left_deep_shuffle_join_scan_count_manual_node1_shuffle_tables.log`、`build/test_left_deep_shuffle_join_scan_count_manual_node2_shuffle_tables.log`、`build/test_left_deep_shuffle_join_later_using_manual_fixed_node1_shuffle_tables.log`、`build/test_left_deep_shuffle_join_later_using_manual_fixed_node2_shuffle_tables.log`、`build/test_left_deep_shuffle_join_second_stage_any_manual_node1_shuffle_tables.log`、`build/test_left_deep_shuffle_join_second_stage_any_manual_node2_shuffle_tables.log`、`build/test_left_deep_shuffle_join_using_chain_manual_node1_shuffle_tables.log`、`build/test_left_deep_shuffle_join_using_chain_manual_node2_shuffle_tables.log`、`build/test_left_deep_shuffle_join_outer_fallback_manual_node1_shuffle_tables.log`、`build/test_left_deep_shuffle_join_outer_fallback_manual_node2_shuffle_tables.log`、`build/test_left_deep_shuffle_join_order_limit_manual_node1_shuffle_tables.log` 和 `build/test_left_deep_shuffle_join_order_limit_manual_node2_shuffle_tables.log` 均为空。完整 Praktika `test_distributed_shuffle_join` 本轮尝试日志见 `build/test_integration_distributed_shuffle_join_left_deep_full.log`；最新重试日志见 `build/test_integration_distributed_shuffle_join_left_deep_retry.log` 和 `build/test_integration_distributed_shuffle_join_left_deep_retry_tmp.log`，其中普通运行仍因 `ci/tmp/environment.json` 权限失败，改用仓库 `tmp/praktika` 后进入 Docker 阶段但因无法访问 `/var/run/docker.sock` 失败；提升运行请求被安全策略拒绝，因为该操作需要 Docker socket 和挂载整个工作区。新增的第二阶段 Exchange 失败 output cleanup、多表 `LEFT SEMI JOIN` fallback 和左深隐藏 `ORDER BY` 用例已通过 Python AST 检查，但尚未完成 Praktika 验证。
...
- 当前已初步支持保守的左深树多表分布式 `INNER ALL JOIN` / 普通 `INNER JOIN` / `INNER ANY JOIN`：按 SQL 原始 join 顺序拆成多个二元 `shuffle join` stage，中间结果写入普通 `Memory` output 表，下一 stage 再继续 shuffle；deterministic `WHERE` conjunct 会在所有引用表已经可用的最早 stage 执行，且只用于早期过滤的列不会保留进后续 output schema；重复中间列名会通过内部列名映射处理。多表主线设计记录在 `docs/distributed_shuffle_join_multi_table_design.md`；同 key multi-way shuffle 仍作为后续优化，而不是多表正确性的前置条件。

下一步建议：

1. 为左深多阶段路径补充 analyzer/gtest 覆盖，验证三表和 N 表规划、stage output schema、重复列名映射，以及非 `INNER` 多表查询回退普通 planner。
2. 跑完整 Praktika `test_distributed_shuffle_join`，确认新增三表和四表 integration 在标准 Docker 环境中通过，并继续检查每个 source shard 每侧输入只扫描一次。
3. 多阶段路径稳定后，增加 partitioning property 传递，判断上一 stage output 是否已经按下一 stage key 分布，从而避免不必要的 reshuffle。
